...
function disliker()
{
    wp_enqueue_script('idev_unlike_post', plugins_url('disliker.ajax.js', __FILE__), ['jquery'], '1.0', 1);
    wp_localize_script('idev_unlike_post', 'ajax_var', [
        ...
        if (is_user_logged_in()) { // user is logged in
            ...
            if (!AlreadyLikedPost($post_id)) {
                if (!AlreadyUnLiked($post_id)) { // like the post
                    ...
                    echo $post_unlike_count.' dislikes'; // update count on front end
                    ...
                    update_user_option($user_id, '_user_unlike_count', $user_likes); // -1 count user meta
                    echo 'count'.$post_unlike_count.' dislikes'; // update count on front end
                }
            } else {
...
function AlreadyUnLiked($post_id)
...
function AlreadyLikedPost($post_id)
{ // test if user liked before
    if (is_user_logged_in()) { // user is logged in
        $user_id = get_current_user_id(); // current user
...
function getPostUnLikeLink($post_id)
{
    ...
    $output = '<a href="#" class="idev-post-unlike'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">&nbsp;'.$count.' dislikes</a>';
...
function show_user_unlikes($user)
{
    ...
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e('You Unlike:');
    ?></label></th>
			<td>
            <?php
